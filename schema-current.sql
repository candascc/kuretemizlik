...
CREATE TABLE recurring_job_occurrences ( id INTEGER PRIMARY KEY AUTOINCREMENT, recurring_job_id INTEGER NOT NULL, company_id INTEGER NOT NULL DEFAULT 1, date TEXT NOT NULL, start_at TEXT NOT NULL, end_at TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'PLANNED', created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), `job_id` INTEGER, FOREIGN KEY(recurring_job_id) REFERENCES recurring_jobs(id) ON DELETE CASCADE, FOREIGN KEY(company_id) REFERENCES companies(id) ON DELETE CASCADE );
...
CREATE TABLE resident_users ( id INTEGER PRIMARY KEY AUTOINCREMENT, unit_id INTEGER NOT NULL, name TEXT NOT NULL, email TEXT UNIQUE, phone TEXT, password_hash TEXT, password_set_at TEXT, last_otp_sent_at TEXT, otp_attempts INTEGER NOT NULL DEFAULT 0, otp_context TEXT, is_owner INTEGER DEFAULT 1, is_active INTEGER DEFAULT 1, email_verified INTEGER DEFAULT 0, phone_verified INTEGER DEFAULT 0, verification_token TEXT, email_verified_at TEXT, phone_verified_at TEXT, last_login_at TEXT, created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now')), FOREIGN KEY(unit_id) REFERENCES units(id) ON DELETE CASCADE );
CREATE INDEX idx_resident_users_phone ON resident_users(phone);

-- TABLE: resident_login_tokens
CREATE TABLE resident_login_tokens ( id INTEGER PRIMARY KEY AUTOINCREMENT, resident_user_id INTEGER NOT NULL, token TEXT NOT NULL, channel TEXT NOT NULL CHECK(channel IN ('email','sms')), expires_at TEXT NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, max_attempts INTEGER NOT NULL DEFAULT 5, meta TEXT, consumed_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), FOREIGN KEY(resident_user_id) REFERENCES resident_users(id) ON DELETE CASCADE );
CREATE INDEX idx_resident_login_tokens_user ON resident_login_tokens(resident_user_id);
CREATE INDEX idx_resident_login_tokens_token ON resident_login_tokens(token);
CREATE INDEX idx_resident_login_tokens_expires ON resident_login_tokens(expires_at);

-- TABLE: customer_login_tokens
CREATE TABLE customer_login_tokens ( id INTEGER PRIMARY KEY AUTOINCREMENT, customer_id INTEGER NOT NULL, token TEXT NOT NULL, channel TEXT NOT NULL CHECK(channel IN ('email','sms')), expires_at TEXT NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, max_attempts INTEGER NOT NULL DEFAULT 5, meta TEXT, consumed_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), FOREIGN KEY(customer_id) REFERENCES customers(id) ON DELETE CASCADE );
...
-- INDEX: idx_recurring_occurrences_company_id
CREATE INDEX idx_recurring_occurrences_company_id ON recurring_job_occurrences(company_id);
