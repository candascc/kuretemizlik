...
$resendCooldown = $flow['resend_cooldown'] ?? CustomerOtpService::RESEND_COOLDOWN_SECONDS;
...
<head>
    ...
    <title><?= htmlspecialchars($title ?? 'Müşteri Portalı') ?></title>
        ...
        <div class="w-full max-w-4xl">
            <div class="flex justify-center mb-6">
                <span class="block h-28 w-28 rounded-full overflow-hidden">
                    <!-- Performance: WebP support with fallback, responsive sizes -->
                    <picture>
                        <source srcset="<?= Utils::asset('img/logokureapp.webp') ?>" type="image/webp">
                        <img src="<?= Utils::asset('img/logokureapp.png') ?>" 
                             width="120" height="120" 
                             alt="Küre Temizlik Logosu"
            ...
            <div class="relative grid gap-8 rounded-3xl bg-white p-8 shadow-xl ring-1 ring-gray-100 dark:bg-gray-900 dark:ring-gray-800 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12">
                <div class="flex flex-col gap-6">
                    <header class="space-y-3 text-center sm:text-left">
                        <p class="inline-flex items-center justify-center gap-2 rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-blue-700 dark:bg-blue-900/30 dark:text-blue-200">
                            <i class="fas fa-user-circle"></i>
                            Müşteri Portalı
                        </p>
                        <h1 class="text-3xl font-semibold text-gray-900 dark:text-white"><?= e($stepTitle) ?></h1>
                        <p class="text-sm leading-6 text-gray-600 dark:text-gray-400">
                            Kayıtlı telefon numaranızla giriş yapabilir, ilk girişinizde birkaç dakika içinde şifrenizi belirleyebilirsiniz.
                        </p>
                    </header>
                    ...
                    <?php if ($step === 'phone'): ?>
                        <form method="POST"
                              action="<?= base_url('/portal/login') ?>"
                              class="space-y-6"
                              data-login-form
                              data-default-action="<?= base_url('/portal/login') ?>">
                            <?= CSRF::field() ?>
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    class="mt-1 block w-full rounded-xl border <?= $phoneError ? 'border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-red-600' : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700' ?> bg-white px-4 py-3 text-base text-gray-900 placeholder:text-gray-400 shadow-sm transition dark:bg-gray-800 dark:text-gray-100"
                                ...
                                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400 <?= $phoneError ? 'hidden' : '' ?>"
                                   data-phone-helper
                                   data-default-message="Yalnızca sistemde kayıtlı müşteri telefon numaralarına doğrulama kodu gönderilir.">
                                    Yalnızca sistemde kayıtlı müşteri telefon numaralarına doğrulama kodu gönderilir.
                                </p>
                            ...
                            <div class="space-y-3">
                                ...
                                <button type="button"
                                        class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white py-3 text-sm font-semibold text-blue-600 shadow-sm transition hover:bg-blue-50 hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:ring-offset-2 focus:ring-offset-white dark:border-blue-900/40 dark:bg-gray-900 dark:text-blue-200 dark:hover:bg-blue-900/10 dark:focus:ring-offset-gray-900"
                                        data-forgot-trigger
                                        data-forgot-action="<?= base_url('/portal/login/forgot') ?>">
                    ...
                    <?php elseif ($step === 'password'): ?>
                        ...
                        <form method="POST" action="<?= base_url('/portal/login/password') ?>" class="space-y-6">
                            <?= CSRF::field() ?>
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    class="mt-1 block w-full rounded-xl border <?= $passwordError ? 'border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-red-600' : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700' ?> bg-white px-4 py-3 text-base text-gray-900 placeholder:text-gray-400 shadow-sm transition dark:bg-gray-800 dark:text-gray-100"
                                    placeholder="••••••••">
                            ...
                            <div class="space-y-3">
                                ...
                                <button type="submit"
                                        class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white py-3 text-sm font-semibold text-blue-600 shadow-sm transition hover:bg-blue-50 hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:ring-offset-2 focus:ring-offset-white dark:border-blue-900/40 dark:bg-gray-900 dark:text-blue-200 dark:hover:bg-blue-900/10 dark:focus:ring-offset-gray-900"
                                        formaction="<?= base_url('/portal/login/forgot') ?>"
                                        formmethod="post">
                                    <i class="fas fa-key"></i>
                                    Şifremi Unuttum
                                </button>
                        ...
                        <form method="POST" action="<?= base_url('/portal/login/cancel') ?>" class="text-center">
                            <?= CSRF::field() ?>
                            <button type="submit" class="text-xs font-semibold text-gray-500 transition hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-300">
                                <i class="fas fa-rotate-left"></i>
                                Farklı bir telefon numarası dene
                            </button>
                        </form>
                    <?php elseif ($step === 'otp'): ?>
                        <div class="rounded-2xl border border-blue-200 bg-blue-50/80 p-4 text-sm text-blue-700 dark:border-blue-900/40 dark:bg-blue-900/10 dark:text-blue-200">
                        ...
                        <form method="POST" action="<?= base_url('/portal/login/otp') ?>" class="space-y-6">
                            <?= CSRF::field() ?>
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    inputmode="numeric"
                                    pattern="[0-9]*"
                                    maxlength="<?= CustomerOtpService::OTP_LENGTH ?>"
                                    data-otp-input
                                    data-auto-focus
                                    aria-invalid="<?= $otpError ? 'true' : 'false' ?>"
                                    class="mt-1 block w-full rounded-xl border <?= $otpError ? 'border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-red-600' : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700' ?> bg-white px-4 py-3 text-center text-xl tracking-widest text-gray-900 placeholder:text-gray-300 shadow-sm transition focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:text-gray-100"
                                    placeholder="<?= str_repeat('•', CustomerOtpService::OTP_LENGTH) ?>">
                        ...
                        <form method="POST" action="<?= base_url('/portal/login/resend') ?>" class="mt-4 text-center" data-resend-form>
                            <?= CSRF::field() ?>
                            <button type="submit"
                                    class="inline-flex items-center gap-2 text-xs font-semibold text-blue-600 transition hover:text-blue-500 disabled:cursor-not-allowed disabled:text-gray-400 dark:text-blue-300 dark:hover:text-blue-200"
                                    data-resend-button
                    ...
                    <?php elseif ($step === 'set_password'): ?>
                        ...
                        <form method="POST" action="<?= base_url('/portal/login/set-password') ?>" class="space-y-6">
                            <?= CSRF::field() ?>
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    class="mt-1 block w-full rounded-xl border <?= $setPasswordError ? 'border-red-300 focus:border-red-500 focus:ring-red-500 dark:border-red-600' : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700' ?> bg-white px-4 py-3 text-base text-gray-900 placeholder:text-gray-400 shadow-sm transition focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:text-gray-100"
                            ...
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    type="password"
                                    autocomplete="new-password"
                                    class="mt-1 block w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-base text-gray-900 placeholder:text-gray-400 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">Telefon numaranız değiştiyse müşteri temsilcinizle iletişime geçin.</p>
                </div>
                ...
                <aside class="flex flex-col justify-between gap-6 rounded-2xl border border-gray-200 bg-gray-50 p-6 dark:border-gray-800 dark:bg-gray-900/70">
                    ...
                    <div class="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-sm text-blue-700 shadow-sm dark:border-blue-900/40 dark:bg-blue-900/20 dark:text-blue-200">
                        <p class="font-semibold">Destek</p>
                        <p class="mt-2 leading-6">Kod ulaşmazsa <?= $resendCooldown ?> saniye sonra yeniden deneyin veya müşteri temsilcimizle iletişime geçin.</p>
                    </div>
                </aside>
            </div>
        </div>
    ...
    (function () {
        ...
        if (otpInput) {
            otpInput.addEventListener('input', (event) => {
                const cleaned = event.target.value.replace(/\D/g, '').slice(0, <?= CustomerOtpService::OTP_LENGTH ?>);
